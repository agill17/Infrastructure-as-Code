...
variable "type" { default = "t2.medium" }
variable "ami" { default = ""}
variable "key_name" { type = "string"}
variable "security_groups" { type = "list"}
variable "subnet_id" { type = "string"}
variable "associate_public_ip_address" { default = true }
variable "availability_zone" {default = "us-east-1d"}
...
variable "private_key" { type = "string" description= "path to private key file"}
variable "user_data" {
  type = "string"
...
resource "aws_instance" "ec2" {
  ...
  tags = {Name = "${var.env}-${var.app_name}-${count.index}"}
